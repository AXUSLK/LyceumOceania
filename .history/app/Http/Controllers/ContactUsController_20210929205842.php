...
class ContactUsController extends Controller
{
    ...
    public function ContactUs(Request $request)
    {
        ...
        $beautymail->send('emails.newIssue', ["project" => $name], function($message) use($$message, $email, $name)
        $beautymail->send('emails.contact_us', [], function ($message) use($message, $email, $name){
            $message
                ->from($email, $name)
